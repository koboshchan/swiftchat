...
actor SignedOutChatProvider: ChatProvider {
    func bootstrap() async throws -> BootstrapSnapshot { throw ChatProviderError.unauthenticated }
    func channels(in guildID: GuildID?) async throws -> [Channel] { throw ChatProviderError.unauthenticated }
    func members(in guildID: GuildID?) async throws -> [Member] { throw ChatProviderError.unauthenticated }
    ...
    func updateStatus(_ status: PresenceStatus) async throws { throw ChatProviderError.unauthenticated }
    func messages(in channelID: ChannelID, before: MessageID?, limit: Int) async throws -> MessagePage {
        throw ChatProviderError.unauthenticated
    }
    func send(_ draft: SendMessageDraft) async throws -> Message { throw ChatProviderError.unauthenticated }
