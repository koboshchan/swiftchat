...
struct MockChatFixture {
    ...
    static func make(now: Date = .now, includesLongServerList: Bool = false) -> Self {
        ...
        let longListGuilds = includesLongServerList ? (0..<18).map { index in
            Guild(
                id: GuildID(rawValue: UInt64(1_000 + index)),
        ...
        channels.append(contentsOf: longListGuilds.enumerated().map { index, guild in
            Channel(
                id: ChannelID(rawValue: UInt64(2_000 + index)),
                guildID: guild.id,
                name: "general",
        ...
        let base = now.addingTimeInterval(-2_700)
        ...
        var messages: [ChannelID: [Message]] = [
            ChannelID(rawValue: 200): [
                message(1_001, 200, rowan, "Welcome to **Aurora Studio** — a fictional community bundled with SwiftChat's offline demo.", base),
                message(1_002, 200, maya, "Everything here is synthetic: people, profiles, conversations, and artwork. Feel free to click around.", base.addingTimeInterval(90)),
            ],
            ChannelID(rawValue: 201): [
                message(1_101, 201, nova, "**Demo build 0.4**\n• compact multiline composer\n• local attachment previews\n• richer profile fixtures", base.addingTimeInterval(180)),
            ],
            ChannelID(rawValue: 202): [
                message(1_201, 202, rowan, "Be curious, give specific feedback, and remember that every profile in this demo is fictional.", base.addingTimeInterval(240)),
            ],
            ChannelID(rawValue: 210): [
                message(2_001, 210, maya, "I tried the new sidebar at three window widths. The compact state finally feels intentional.", base.addingTimeInterval(420)),
                message(2_002, 210, juniper, "Nice. I also checked keyboard navigation — focus stays put when the member list opens.", base.addingTimeInterval(485), reactions: [Reaction(emoji: "✅", count: 3)]),
                message(2_003, 210, theo, "The little server artwork makes a surprisingly big difference. No more mystery squares.", base.addingTimeInterval(610)),
                message(2_004, 210, nova, "Agreed. I kept the fallback neutral so unnamed test servers still look deliberate.", base.addingTimeInterval(665)),
                message(2_005, 210, maya, "Next pass: make the empty channel state feel as polished as the busy one?", base.addingTimeInterval(840)),
                message(2_006, 210, juniper, "Already added it to the fictional backlog ✨", base.addingTimeInterval(900), reactions: [Reaction(emoji: "✨", count: 4)]),
            ],
            ChannelID(rawValue: 211): [
                message(2_101, 211, maya, "Design note: toolbar identity should answer “where am I?” without competing with the channel title.", base.addingTimeInterval(520)),
                message(2_102, 211, nova, "I’m using the server mark first, then the channel control. Both stay readable when the window narrows.", base.addingTimeInterval(590)),
                message(2_103, 211, rowan, "The placeholder also needs a proper accessibility label for unnamed servers.", base.addingTimeInterval(690)),
            ],
            ChannelID(rawValue: 212): [
                message(2_201, 212, juniper, "Does anyone have a clean pattern for sizing an `NSTextView` inside `NSViewRepresentable`?", base.addingTimeInterval(600)),
                message(2_202, 212, nova, "Measure the layout manager with the proposed width, but clamp the usable width before laying out. Zero-width proposals can explode the height.", base.addingTimeInterval(720)),
                message(2_203, 212, theo, "That explains a composer I once saw become approximately one kilometre tall.", base.addingTimeInterval(780), reactions: [Reaction(emoji: "😅", count: 2)]),
            ],
            ChannelID(rawValue: 220): [
                message(2_301, 220, maya, "**Suggestion:** keep demo data isolated from account caches so screenshots are repeatable.", base.addingTimeInterval(500)),
                message(2_302, 220, nova, "Implemented with an in-memory demo database. Nothing carries between launches.", base.addingTimeInterval(760)),
            ],
            ChannelID(rawValue: 221): [
                message(2_401, 221, juniper, "**Resolved:** empty composer opened at maximum height after an initial zero-width layout pass.", base.addingTimeInterval(560)),
            ],
            ChannelID(rawValue: 300): [
                message(3_001, 300, theo, "Instruments found a 14% drop in idle rendering work after splitting the animated status row.", base.addingTimeInterval(300)),
                message(3_002, 300, nova, "That lines up with the observation scopes. Small leaf views are doing their job.", base.addingTimeInterval(390)),
                message(3_003, 300, maya, "And it still reads like ordinary SwiftUI instead of a framework inside a framework.", base.addingTimeInterval(470)),
            ],
            ChannelID(rawValue: 301): [
                Message(
                    id: MessageID(rawValue: 3_101),
            ...
            ChannelID(rawValue: 302): [
                message(3_201, 302, juniper, "Cold launch is stable across five runs. I’m moving on to resize stress tests.", base.addingTimeInterval(700)),
                message(3_202, 302, theo, "Try rapid inspector toggles too; that used to reveal layout churn immediately.", base.addingTimeInterval(790)),
            ],
            ChannelID(rawValue: 400): [
                message(4_001, 400, maya, "Hey! I left a layout study in #showcase when you have a minute.", base.addingTimeInterval(980)),
                message(4_002, 400, nova, "Just saw it — the hierarchy is much clearer. I’ll try the tighter spacing.", base.addingTimeInterval(1_080)),
                message(4_003, 400, maya, "Perfect. No rush; this entire conversation is made of demo pixels anyway 🙂", base.addingTimeInterval(1_140)),
            ],
        ]
        for (index, guild) in longListGuilds.enumerated() {
            let channelID = ChannelID(rawValue: UInt64(2_000 + index))
            messages[channelID] = [message(
                ...
                base.addingTimeInterval(Double(1_200 + index * 30))
            )]
        }
