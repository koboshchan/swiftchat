  ...
  switch (id) {
    ...
    default:
      throw std::runtime_error(
        "Unsupported KEM algorithm ID " + std::to_string(static_cast<uint16_t>(id)));
  }
}
...
select_kdf(KDF::ID id)
{
  switch (id) {
    ...
    default:
      throw std::runtime_error(
        "Unsupported KDF algorithm ID " + std::to_string(static_cast<uint16_t>(id)));
  }
}
...
select_aead(AEAD::ID id)
{
  switch (id) {
    ...
    default:
      throw std::runtime_error(
        "Unsupported AEAD algorithm ID " + std::to_string(static_cast<uint16_t>(id)));
  }
}
