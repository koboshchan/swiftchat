...
struct ECKeyGroup : public EVPGroup
{
    ...
    const typed_unique_ptr<BIGNUM>& priv) const
  {
    ...
    if (name == nullptr) {
      throw std::runtime_error("Unsupported algorithm: EC curve NID has no short name");
    }
  ...
  typed_unique_ptr<EVP_PKEY> public_evp_key(const bytes& pub) const
  {
    const auto* name = OBJ_nid2sn(curve_nid);
    if (name == nullptr) {
      throw std::runtime_error("Unsupported algorithm: public EC curve NID has no short name");
    }
...
private:
  ...
  static int group_to_nid(Group::ID group_id)
  {
    switch (group_id) {
      ...
      default:
        throw std::runtime_error("Unsupported algorithm: group is not a supported prime curve");
    }
  }
  ...
  uint8_t bitmask() const
  {
    switch (id) {
      ...
      default:
        throw std::runtime_error("Unsupported algorithm: group has no scalar bitmask");
    }
  }
};
...
private:
  ...
  static int group_to_evp(Group::ID group_id)
  {
    switch (group_id) {
      ...
      default:
        throw std::runtime_error("Unsupported algorithm: group is not a supported raw curve");
    }
  }
};
